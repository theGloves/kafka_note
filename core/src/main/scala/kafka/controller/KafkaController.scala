...
class ControllerContext(val zkUtils: ZkUtils,
  ...
  val controllerLock: ReentrantLock = new ReentrantLock() // 在哪里的锁
  var shuttingDownBrokerIds: mutable.Set[Int] = mutable.Set.empty // 正在关闭捯Broker list
  val brokerShutdownLock: Object = new Object // broker关闭操作的互斥锁
  var epoch: Int = KafkaController.InitialControllerEpoch - 1 // controller epoch 隔离。
  var epochZkVersion: Int = KafkaController.InitialControllerEpochZkVersion - 1 // ？
  var allTopics: Set[String] = Set.empty // 所有topic的信息，string存的什么？
  var partitionReplicaAssignment: mutable.Map[TopicAndPartition, Seq[Int]] = mutable.Map.empty // 分区replica分配信息？
  var partitionLeadershipInfo: mutable.Map[TopicAndPartition, LeaderIsrAndControllerEpoch] = mutable.Map.empty // 分区leader信息
  val partitionsBeingReassigned: mutable.Map[TopicAndPartition, ReassignedPartitionsContext] = new mutable.HashMap // 分区正在重调整list
  val partitionsUndergoingPreferredReplicaElection: mutable.Set[TopicAndPartition] = new mutable.HashSet // ？

  private var liveBrokersUnderlying: Set[Broker] = Set.empty // 活跃Broker underlying指什么？
